...
def detectLines(img):
    ...
    detected_lines_H = cv2.morphologyEx(
        thresh, cv2.MORPH_OPEN, horizontal_kernel, iterations=2)
    cntsH = cv2.findContours(
        detected_lines_H, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    ...
    detected_lines_V = cv2.morphologyEx(
        thresh, cv2.MORPH_OPEN, vertical_kernel, iterations=2)
    cntsV = cv2.findContours(
        detected_lines_V, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
